...
int applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact) {
  ...
  *h = MAX(1, *h);
  ...
  if (*w < bh)
    *w = bh;
  if (resizehints || c->isfreesize || c->isfloating ||
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) { /* status is only drawn on selected monitor */
    ...
    apply_fribidi(stext);
    tw = status2dtextlength(fribidi_text);
  }
  resizebarwin(m);
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    if (selmon->alttag) {
      if (masterclientontag[i])
        snprintf(tagdisp, 64, ptagf, fribidi_text, masterclientontag[i]);
      else
        snprintf(tagdisp, 64, etagf, fribidi_text);
    ...
    if (n > 0) {
      ...
      for (c = m->clients; c; c = c->next) {
        ...
        if (tw > 0) /* trap special handling of 0 in drw_text */
          drw_text(drw, x, 0, tw, bh, lrpad / 2, fribidi_text, 0);
        if (m->sel == c)
  ...
  do {
    ...
    case MotionNotify:
      ...
      if (abs(selmon->wx - nx) < snap)
        ...
        nx = selmon->wx + selmon->ww - WIDTH(c) - gappoh;
      if (abs(selmon->wy - ny) < snap)
        ny = selmon->wy + gappov;
      else if (abs((selmon->wy + selmon->wh) - (ny + HEIGHT(c))) < snap)
        ny = selmon->wy + selmon->wh - HEIGHT(c) - gappov;
...
void updatestatus(void) {
  if (!gettextprop(root, XA_WM_NAME, rawstext, sizeof(rawstext)))
    strcpy(stext, "macy's dwm-" VERSION " ");
