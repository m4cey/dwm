...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isfreesize, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, issticky;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact)
{
	...
	*h = MAX(1, *h);
	...
	if (*w < bh)
		*w = bh;
	if (resizehints || c->isfreesize  || c->isfloating || !c->mon->lt[c->mon->sellt]->arrange) {
...
clientmessage(XEvent *e)
{
  ...
  if (showsystray && cme->window == systray->win && cme->message_type == netatom[NetSystemTrayOP]) {
    ...
    if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
      ...
      Clr tempClr;
      drw_clr_create(drw, &tempClr, termcolor[2]);
      drw_setscheme(drw, &tempClr);
      swa.background_pixel = tempClr.pixel;
...
updatesystray(void)
{
  ...
  drw_clr_create(drw, &tempClr, termcolor[2]);
    ...
    drw_setscheme(drw, &tempClr);
		wa.background_pixel  = scheme[SchemeNorm][ColBg].pixel; //tempClr.pixel;
